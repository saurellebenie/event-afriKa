...
export default function AIEventPreview() {
  ...
  const generatePreview = useCallback(() => {
    ...
    const eventLabel = eventTypes.find(e => e.id === selection.eventType)?.[locale === 'fr' ? 'labelFr' : 'labelEn']
    const decorLabel = decorStyles.find(d => d.id === selection.decorStyle)?.[locale === 'fr' ? 'labelFr' : 'labelEn']
    const colorLabel = colorPalettes.find(c => c.id === selection.colorPalette)?.[locale === 'fr' ? 'labelFr' : 'labelEn']

    const prompt = encodeURIComponent(
        `Professional event photography, ${decorLabel} ${eventLabel}, ${colorLabel} color scheme, elegant table settings, flowers, ambient lighting, ultra realistic`
    )
